...
```text
โจทยให้รับวันเเละเดือนมา เเละให้บอกว่าวันเเละเดือนที่ให้มาว่าอยู่ใน ฤดูไหน

input:
 รับค่าวันเเละเดือนทีละบรรทัด

output :
เเสดงออกมาเป็นฤดูที่อยู่ในช่วงวันเเละเดือนนั้นๆ

constraints:
ตั้งแต่วันที่ 21 ของเดือนที่ 3 หารลงตัว ฤดูกาลจะเปลี่ยนไป
```
...
```text
Step 1: รับค่า วันเเละเดือน อย่างละบรรทัด
Step 2: ตรวจสอบว่า ช่วงเดือน ตรงกับฤดูไหน
Step 3: ตรวจสอบว่า หาก เดือนคือ 3,6,9,12 เเละ >= 21 หรือไม่
```
...
```text
เหมือนเดิม ใช้การกำหนดเดือนที่อยู่ระหว่างฤดูไปเลย เพราะว่า เดือนมีเเค่ 4 เดือน ที่ใช้เงื่อนไข >= 21
```
...
```text
กรณีที่อยู่ใน วันที่เปลี่ยนฤดูพอดี
```
...
```text
กรณีที่อยู่ใน วันที่เปลี่ยนฤดูพอดี เเต่เดือนยังไม่ถึง
```
